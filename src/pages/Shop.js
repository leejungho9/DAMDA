...
function Shop() {
  const [selectCategory, setSelectCategory] = useState(["수제 청", "수제 꿀", "즙","건강식품","건강디저트"])
  const [isCategory, setCategory] = useState("수제 청")
  
  const [selectFilter, setSelectFilter] = useState(["리뷰순","클릭순","구입순"]);
  ...
  function CreaedButton({ item,menuIdx }) {
      return item.map((el,idx) => {
        return (
          <RadioContainer key={idx} onClick={menuIdx===0?handleCategory:handleFilter}>
          <RadioButton
          id={el}
          value={el}
          ...
          checked={(menuIdx===0?isCategory:isFilter) === el} /><RadioTitle>{el}</RadioTitle>
          </RadioContainer>
        )
    })
  }
  return (
    ...
    <ShopWrapper>
      <MenuWrapper>
        <CategoryContainer><MenuTitle>CATEGORY</MenuTitle><SplitLine />
        <CreaedButton item={selectCategory} menuIdx={0}/>
        </CategoryContainer>
        <FilterContainer><MenuTitle>FILTER</MenuTitle><SplitLine />
